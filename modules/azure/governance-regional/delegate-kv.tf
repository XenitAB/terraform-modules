...
resource "azurerm_key_vault" "delegate_kv" {
  ...
  name = each.value.disable_unique_suffix == true ? join("-", compact(["kv-${var.environment}-${var.location_short}-${each.value.common_name}", var.unique_suffix])) : join("-", compact(["kv-${var.environment}-${var.location_short}-${each.value.common_name}", var.unique_suffix]))
