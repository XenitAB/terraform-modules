...
resource "azuread_group_member" "service_endpoint_join_spn" {
  ...
  group_object_id  = data.azuread_group.service_endpoint_join.id
...
resource "azuread_group_member" "service_endpoint_join_owner" {
  ...
  group_object_id  = data.azuread_group.service_endpoint_join.id
  member_object_id = data.azuread_group.rg_owner[each.key].id
}
...
resource "azuread_group_member" "service_endpoint_join_contributor" {
  ...
  group_object_id  = data.azuread_group.service_endpoint_join.id
  member_object_id = data.azuread_group.rg_contributor[each.key].id
}
