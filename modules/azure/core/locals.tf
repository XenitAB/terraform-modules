...
locals {
  subnets = [
    for subnet in var.vnet_config.subnets : {
      ...
      delegations              = subnet.delegations == null ? [] : subnet.delegations
    }
  ]
  ...
  security_rules = flatten([
    for subnet in var.vnet_config.subnets : [
      for security_rule in subnet.security_rules : {
        ...
        rule_source_address_prefixes      = security_rule.source_address_prefixes
        rule_destination_address_prefix   = security_rule.destination_address_prefix
        rule_destination_address_prefixes = security_rule.destination_address_prefixes
