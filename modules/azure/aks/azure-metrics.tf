...
resource "azurerm_role_assignment" "azure_metrics_aks_reader" {
  scope = "/subscriptions/${data.azurerm_client_config.current.subscription_id}/resourcegroups/${data.azurerm_resource_group.this.name}/providers/Microsoft.ContainerService/managedClusters/aks-${var.environment}-${var.location_short}-${var.name}${var.aks_name_suffix != null ? var.aks_name_suffix : ""}"
...
resource "azurerm_role_assignment" "azure_metrics_lb_reader" {
  scope = "/subscriptions/${data.azurerm_client_config.current.subscription_id}/resourceGroups/mc_${data.azurerm_resource_group.this.name}_aks-${var.environment}-${var.location_short}-${var.name}${var.aks_name_suffix != null ? var.aks_name_suffix : ""}_${data.azurerm_resource_group.this.location}/providers/Microsoft.Network/loadBalancers/kubernetes"
  role_definition_name = "Monitoring Reader"
  principal_id         = azurerm_user_assigned_identity.azure_metrics.principal_id
}
