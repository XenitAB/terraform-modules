...
| [azurerm_user_assigned_identity.azure_metrics](https://registry.terraform.io/providers/hashicorp/azurerm/3.99.0/docs/data-sources/user_assigned_identity) | data source |
...
| <a name="input_fluxcd_config"></a> [fluxcd\_config](#input\_fluxcd\_config) | Configuration for FluxCD | <pre>object({<br>    organization = string<br>    github = optional(object({<br>      application_id  = number<br>      installation_id = number<br>      private_key     = string<br>    }))<br>    azure_devops = optional(object({<br>      pat = string<br>    }))<br>  })</pre> | n/a | yes |
...
| <a name="input_namespaces"></a> [namespaces](#input\_namespaces) | The namespaces that should be created in Kubernetes. | <pre>list(<br>    object({<br>      name   = string<br>      labels = map(string)<br>      fluxcd = optional(object({<br>        provider    = string<br>        project     = optional(string)<br>        repository  = string<br>        create_crds = optional(bool, false)<br>      }))<br>    })<br>  )</pre> | n/a | yes |
