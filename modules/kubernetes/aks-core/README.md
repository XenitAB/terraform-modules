...
| <a name="module_grafana_agent_tenant"></a> [grafana\_agent\_tenant](#module\_grafana\_agent\_tenant) | ../../kubernetes/grafana-agent-tenant | n/a |
...
| [azurerm_resource_group.core](https://registry.terraform.io/providers/hashicorp/azurerm/2.99.0/docs/data-sources/resource_group) | data source |
...
| <a name="input_grafana_agent_config"></a> [grafana\_agent\_config](#input\_grafana\_agent\_config) | Configuration for Grafana Agent | <pre>object({<br>    remote_logs_url      = string<br>    azure_key_vault_name = string<br>    identity = object({<br>      client_id   = string<br>      resource_id = string<br>      tenant_id   = string<br>    })<br>  })</pre> | n/a | yes |
| <a name="input_grafana_agent_enabled"></a> [grafana\_agent\_enabled](#input\_grafana\_agent\_enabled) | Should Grafana Agent be enabled for platform | `bool` | `true` | no |
| <a name="input_grafana_agent_tenant_config"></a> [grafana\_agent\_tenant\_config](#input\_grafana\_agent\_tenant\_config) | The Grafana Agent configuration for tenants | <pre>object({<br>    remote_write_urls = object({<br>      metrics = string<br>      logs    = string<br>      traces  = string<br>    })<br>    credentials = object({<br>      metrics_username = string<br>      metrics_password = string<br>      logs_username    = string<br>      logs_password    = string<br>      traces_username  = string<br>      traces_password  = string<br>    })<br>    extra_namespaces = list(string)<br>  })</pre> | <pre>{<br>  "credentials": {<br>    "logs_password": "",<br>    "logs_username": "",<br>    "metrics_password": "",<br>    "metrics_username": "",<br>    "traces_password": "",<br>    "traces_username": ""<br>  },<br>  "extra_namespaces": [],<br>  "remote_write_urls": {<br>    "logs": "",<br>    "metrics": "",<br>    "traces": ""<br>  }<br>}</pre> | no |
| <a name="input_grafana_agent_tenant_enabled"></a> [grafana\_agent\_tenant\_enabled](#input\_grafana\_agent\_tenant\_enabled) | Should Grafana Agent be enabled for tenant. 'grafana\_agent\_enabled' must also be set to 'true' | `bool` | `false` | no |
