...
| <a name="input_kube_state_metrics_namepsaces"></a> [kube\_state\_metrics\_namepsaces](#input\_kube\_state\_metrics\_namepsaces) | Comma-separated list of namespaces to be enabled. To get metrics from all namespaces use '' | `string` | `"aad-pod-identity,azad-kube-proxy,calico-system,cert-manager,csi-secrets-store-provider-azure,external-dns,falco,flux-system,gatekeeper-system,goldpinger,ingress-healthz,ingress-nginx,kube-node-lease,kube-public,kube-system,prometheus,reloader,tigera-operator,xenit-system"` | no |
