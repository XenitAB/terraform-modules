...
resource "azurerm_policy_set_definition" "xks" {
  ...
  policy_definition_reference {
    ...
    parameter_values     = <<VALUE
    {
      ...
      "excludedNamespaces": {
        "value": "[concat(parameters('excludedNamespaces'),createArray(${local.system_namespaces}, ${local.exclude_namespaces}),createArray('aad-pod-identity','ambassador','azure-metrics','cert-manager','controle-plane-logs','csi-secrets-store-provider-azure','datadog','external-dns','falco','flux-system','ingress-nginx','prometheus','reloader','spegel','vpa'))]"
      },
      "excludedImages": {
