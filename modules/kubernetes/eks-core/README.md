...
| <a name="input_kube_state_metrics_namepsaces"></a> [kube\_state\_metrics\_namepsaces](#input\_kube\_state\_metrics\_namepsaces) | Comma-separated list of namespaces to be enabled. To get metrics from all namespaces use '' | `string` | `"azad-kube-proxy,calico-system,cert-manager,cluster-autoscaler,csi-secrets-store-provider-aws,external-dns,falco,flux-system,gatekeeper-system,goldpinger,ingress-healthz,ingress-nginx,kube-node-lease,kube-public,kube-system,prometheus,reloader,tigera-operator,xenit-system"` | no |
