...
| <a name="input_prometheus_config"></a> [prometheus\_config](#input\_prometheus\_config) | Configuration for prometheus | <pre>object({<br>    tenant_id            = string<br>    remote_write_url     = string<br>    azure_key_vault_name = string<br>    identity = object({<br>      client_id   = string<br>      resource_id = string<br>      tenant_id   = string<br>    })<br>    volume_claim_size  = optional(string, "10Gi")<br>    resource_selector  = optional(list(string), ["platform"])<br>    namespace_selector = optional(list(string), ["platform"])<br>  })</pre> | n/a | yes |
