...
### ⚠️ Breaking Changes
- Bump `required_version` on all modules from `>= 1.3.0` (and `>= 1.1.7` on the xkf-governance modules) to `>= 1.11.0`. The Key Vault RBAC opt-in uses the OpenTofu 1.11 [`enabled` meta-argument](https://opentofu.org/docs/v1.11/language/meta-arguments/enabled/); the bump is applied repo-wide for consistency.
